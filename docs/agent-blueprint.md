...
| **Agent-governing config** — `.claude/` (hooks, permissions, skills, agents) and CI workflows | The context layer governs the agents; an agent-authored edit to its own instructions or gates is the fox reviewing the henhouse. §5.6's flywheel routes proposals here as small delta PRs — proposals flow freely, the merge is human. |
...
| Checkpointer | Durable run state that survives the session — the design the product's agent loop hand-rolled as an event-sourced step log with resume-by-run-id (`agent/state.py`); the two layers land on the same pattern |
