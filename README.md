...
**Demo tunnel** workflow (`.github/workflows/demo-tunnel.yml`): pick a branch (the dropdown
defaults to `master`) and how long to keep it up — 15 minutes to 4 hours. There is an optional
`ref` field for serving a tag or a SHA instead. A dispatch form is rendered from the workflow
file before any runner exists, so that dropdown can only offer branch names committed to the
file; **Demo tunnel branch list** (`demo-tunnel-branches.yml`) keeps them true by rewriting the
list and opening a PR whenever branches come or go. The job installs the app on a runner, seeds the demo
