...
An approval is a recorded human decision: who approved, and the exact bytes they
approved, snapshotted and hashed -- plus the serializer that keeps send decisions
out of the generic review-decision endpoint.
"""
...
class ApprovalDecisionTests(AuthenticatedAPITestCase):
    ...
    def test_undo_voids_the_approval_but_keeps_it_for_audit(self):
        self.assertTrue(hasattr(ReviewDecision, "KIND_APPROVE_SEND"))  # red at skeleton
        action = _pending_action()
